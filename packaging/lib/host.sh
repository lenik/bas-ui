...
#
# Environment:
#   BAS_UI_SRCDIR       project root (default: packaging/../..)
#   BAS_UI_FORCE_LOCAL  skip can-local check (set on a build host by gh-makerelease)
set -euo pipefail

_LIBDIR=$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)
BAS_UI_SRCDIR=${BAS_UI_SRCDIR:-$(cd "$_LIBDIR/../.." && pwd)}
...
find_build_host_file() {
    ...
    for k in $(_kind_fallbacks "$kind"); do
        for base in "$BAS_UI_SRCDIR/.config/bas-ui" "$HOME/.config/bas-ui"; do
...
cmd_dump() {
    local file=${1:-}
    if [[ -z $file ]]; then
        file=$(find_build_host_file "${BAS_UI_PACKAGING:-win32}") || {
    ...
    if ((BH_N > 0)); then
        ...
        echo "last_shell=$(last_shell "${BAS_UI_PACKAGING:-}")"
...
cmd_run() {
    ...
    local k
    for k in $(_kind_fallbacks "$kind"); do
        echo "    $BAS_UI_SRCDIR/.config/bas-ui/$k.build-host" >&2
        echo "    $HOME/.config/bas-ui/$k.build-host" >&2
    done
    return 2
}
